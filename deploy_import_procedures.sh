#!/usr/bin/env bash
# =====================================================================
#  deploy_import_procedures.sh
#  Import en masse des procedures/annexes/fiches techniques dans la base.
#
#  - Lit PROCEDURES_PLATEFORME.zip (a deposer dans /opt/registre-gmp).
#  - Sauvegarde la base AVANT (registre.db.<horodatage>.bak sur l'hote).
#  - Copie les fichiers dans le conteneur et importe via l'appli (PROC_DIR).
...
echo "=== Decompression de $ZIP ==="
mkdir -p "$TMP/import"
unzip -q "$ZIP" -d "$TMP/import"
[ -f "$TMP/import/_manifest.csv" ] || { echo "ERREUR : _manifest.csv absent du zip." >&2; exit 1; }

# importeur embarque (s'execute DANS le conteneur, en tant qu'appuser)
cat > "$TMP/import/_import_proc.py" <<'PYEOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import os, sys, csv, secrets, sqlite3, datetime, shutil, unicodedata, mimetypes
...
docker exec "$CONTAINER" rm -rf /app/data/_import 2>/dev/null || true
docker cp "$TMP/import" "$CONTAINER:/app/data/_import"
docker exec -u root "$CONTAINER" chown -R 10001:10001 /app/data/_import

echo "=== Import en base (idempotent) ==="
set +e
docker exec "$CONTAINER" python3 /app/data/_import/_import_proc.py
rc=$?
set -e

echo "=== Nettoyage ==="
docker exec "$CONTAINER" rm -rf /app/data/_import || true
...
echo "=== Termine ==="
echo "Aucun redemarrage necessaire : les procedures sont visibles immediatement (Procedures > Gerer)."
